 ...
 * Removes optional leading colons left from an earlier PDO parameter array.
 *
 * @return array The validated row with clean column-name keys.
 */
function validate_db_row(array $row): array
{
    ...
    foreach ($row as $column => $value) {
        ...
        if (array_key_exists($column, $clean_row)) {
            throw new InvalidArgumentException("Database row contains a duplicate column");
        }
...
function insert(string $table_name, array $data, array $opts = []): array
{
    ...
    foreach ($rows as $index => $row) {
        ...
        $rows[$index] = validate_db_row($row);
    }
 ...
 // Add this function below insert() in lib/db_helpers.php.
/**
...
function update(string $table_name, array $data, array $where_keys = ["id"], array $opts = []): array
{
    ...
    // Accept data keys with or without a leading colon.
    ...
    if (!empty($opts["debug"])) {
        ...
        error_log("UPDATE parameter names: " . var_export($params, true));
    }
...
// Add these functions below update() in lib/db_helpers.php.
/**
...
// lib/db_helpers.php
// Keep the existing connection, CRUD, and select helpers above this section.
...
/** Returns the filters supported by the Esports lists. */
function esports_filter_rules(): array
{
    return [
        "name",
        "category_name",
...
/**
 * Builds the Esports conditions and PDO parameters.
 *
 * @param array $filters Validated esports filter values.
 * @param string $column_prefix Trusted table or alias prefix, including the
 * trailing period, such as "e.".
 * @return array SQL joined by AND and its named parameters.
 */
function build_esports_filter_query(
    array $filters,
    ...
    foreach (["name", "category_name"] as $name) {
    ...
    if (!empty($filters["type"])) {
        $column = db_qualified_identifier("type", $column_prefix);
        $conditions[] = "LOWER(" . $column . ") = :type";
